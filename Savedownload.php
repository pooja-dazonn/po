...
$query="SELECT * from  create_po ORDER BY PO DESC limit 1";
...
while($user=$result->fetch())
{
  ...
  $pdf->cell(59 ,5,'',0,1);
    ...
    $pdf->cell(130 ,5,'',0,0);
    $pdf->cell(28 ,5,'Delivery date:',0,0);
...
$pdf->rect(5,5,200,287,'D');
  ...
  $pdf->Cell(19,8,'TYPE ',1);
  $pdf->Cell(19,8,'FINISH',1);
  $pdf->Cell(12,8,'QTY',1);
  ...
  $query1="SELECT CODE,WM,TM,DM,TYPE,FINISH,COLOUR,QTY,SQM,KGS,RATE,ROUND(QTY*SQM,4) as sqm,ROUND(QTY*KGS,4)as kgs,(QTY*RATE) as TOTAL from po order by QTY DESC limit 401";
...

  {
	...
	$pdf->Cell(19,8,$row['TYPE'],1);
	$pdf->Cell(19,8,$row['FINISH'],1);
	$pdf->Cell(12,8,$row['QTY'],1);
...
while($user=$result->fetch()){
  $pdf->Ln(8);
  $pdf->SetFont('Arial','',10);
  $pdf->Cell(98,8,'QTY TOTAL',1);
 $pdf->Cell(12,8,$user['SUM(QTY)'],1);
    }
